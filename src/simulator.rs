...
impl Simulator {
    ...
    fn execute(&mut self, gesture: &Gesture, state: &mut SimState, tick_us: u64) {
        match gesture {
            ...
            Gesture::BarVibrato { width, rate_hz, ms } => {
                ...
                info!(
                    "  vibrato center={:.1} width={:.2} rate={}Hz for {}ms",
    ...
    fn generate_audio(&mut self, state: &SimState, ts: u64) -> AudioChunk {
        ...
        for (si, open_pitch) in open.iter().enumerate() {
            if !state.string_active[si] {
                continue;
            }
            let freq = midi_to_hz(*open_pitch + bar_fret as f64);
...
static CHORD_VOCAB: &[ChordV] = &[
    ...
    ChordV {
        label: "C#",
        fret: 0.,
        ped: [true, false, false],
        lev: [true, false, false, false, false],
        strings: &[2, 3, 4, 5, 7],
        passing: false,
    },
...
fn pick_next_chord(cur: usize, rng: &mut Rng) -> usize {
    ...
    let weights: Vec<u32> = CHORD_VOCAB
        .iter()
        .enumerate()
        .map(|(i, v)| {
            if i == cur {
...
fn chord_transition(g: &mut Vec<Gesture>, old: &ChordV, new: &ChordV, _rng: &mut Rng, beat: u32) {
    let sq = beat * 2;

    // Release current pedals / levers (quick)
    for i in 0..3 {
        if old.ped[i] {
            g.push(Gesture::PedalRelease { index: i, ms: 150 });
        }
    }
    for i in 0..5 {
        if old.lev[i] {
            g.push(Gesture::LeverRelease { index: i, ms: 150 });
        }
    }
    ...
    if dist > 0.01 {
        ...
        g.push(Gesture::BarSlide {
            to: new.fret,
            ms: slide_ms,
        });
    }

    g.push(Gesture::PickStrings {
        strings: new.strings.to_vec(),
    });
    ...
    for i in 0..3 {
        if new.ped[i] {
            g.push(Gesture::PedalEngage {
                index: i,
                ms: if slow_ped { sq } else { 120 },
            });
        }
    }
    for i in 0..5 {
        if new.lev[i] {
            g.push(Gesture::LeverEngage {
                index: i,
                ms: if slow_lev { sq } else { 120 },
            });
        }
    }
}
...
fn improvise_sequence(seed: u64, total_beats: u32) -> Vec<Gesture> {
    ...
    g.push(Gesture::PickStrings {
        strings: CHORD_VOCAB[0].strings.to_vec(),
    });
    g.push(Gesture::VolumeSwell {
        from: 0.0,
        to: 0.82,
        ms: beat + beat / 2,
    });
    ...
    while elapsed_beats < end_beats {
        ...
        let target = (phrase_peak + rng.range(0, 8) as f32 * 0.01 - 0.04).clamp(0.60, 0.92);
        ...
        cur_idx = next_idx;
        info!(
            "  improv: {} → {} (hold {} beats, vol {:.2})",
            cur.label, next.label, hold_beats, cur_vol
        );
    }
